...
func (client *V3AuthClient) Get(url string, query map[string]string, headers map[string]string) (httpclient.Response, error) {
	headers = client.rejectToken(headers)
	return client.session.Get(url, query, headers)
}
func (client *V3AuthClient) Post(url string, body []byte, headers map[string]string) (httpclient.Response, error) {
	headers = client.rejectToken(headers)
	return client.session.Post(url, body, headers)
}
func (client *V3AuthClient) Delete(url string, headers map[string]string) (httpclient.Response, error) {
	headers = client.rejectToken(headers)
	return client.session.Delete(url, headers)
}

func (client *V3AuthClient) ServiceList() (httpclient.Response, error) {
	url := fmt.Sprintf("%s%s", client.AuthUrl, "/services")
	return client.Get(url, nil, map[string]string{})
}

func (client *V3AuthClient) UserList() (httpclient.Response, error) {
